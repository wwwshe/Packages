...
public extension Date {
    ...
    var dayDiffToday: Int? {
        let interval = self.firstTimeOfDay.timeIntervalSince(Date().firstTimeOfDay)
    ...
    func dayDiff(_ date: Date) -> Int? {
        let interval = date.firstTimeOfDay.timeIntervalSince(self.firstTimeOfDay)
        let days = Int(interval / (60*60*24))
        return days
    }
    ...
    static func betweenTimeStartToday(endTimestamp: Double) -> String {
        ...
        var betweenTime: Int = Int(endTimestamp - nowTimestamp)
